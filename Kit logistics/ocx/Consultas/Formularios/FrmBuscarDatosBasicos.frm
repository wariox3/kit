...
Begin VB.Form FrmConsultaDatosBasicos 
   BorderStyle     =   4  'Fixed ToolWindow
   Caption         =   "Buscar Datos Basicos..."
   ClientHeight    =   4665
   ClientLeft      =   45
   ClientTop       =   285
   ClientWidth     =   6885
   ...
   Begin VB.ComboBox CboCriterio 
      Height          =   315
      ItemData        =   "FrmBuscarDatosBasicos.frx":0000
      Left            =   120
      List            =   "FrmBuscarDatosBasicos.frx":000A
   ...
   End
End
Attribute VB_Name = "FrmConsultaDatosBasicos"
...
Private Sub CmdAceptar_Click()
  If rstTem.EOF = False Then ElSt = rstTem.Fields("ID") & ""
  Unload Me
End Sub
...
Sub Filtrar()
  ...
  Consulta = "SELECT ID, Concat(Nombre,  ' ', Apellido1, ' ', Apellido2) AS NombreCompleto FROM DatosBasicos"
  If TxtBuscamos <> "" And CboCriterio.Text <> "" Then
    If CboCriterio.Text = "Codigo" Then
      If ChkContenga.Value = 1 Then
        Consulta = Consulta + " where ID like '%" & TxtBuscamos.Text & "%' order by NombreCompleto"
      Else
        Consulta = Consulta + " where ID like '" & TxtBuscamos.Text & "%' order by NombreCompleto"
      End If
    End If
    If CboCriterio.Text = "Nombre" Then
      If ChkContenga.Value = 1 Then
        Consulta = Consulta + " where NombreCompleto like '%" & TxtBuscamos.Text & "%' order by NombreCompleto"
      Else
        Consulta = Consulta + " where NombreCompleto like '" & TxtBuscamos.Text & "%' order by NombreCompleto"
      End If
    End If
