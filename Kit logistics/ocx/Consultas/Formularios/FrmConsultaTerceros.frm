...
Begin VB.Form FrmConsultaTerceros 
   BorderStyle     =   4  'Fixed ToolWindow
   Caption         =   "Consulta terceros..."
   ClientHeight    =   4665
   ClientLeft      =   45
   ClientTop       =   285
   ClientWidth     =   6840
   ...
   Begin VB.ComboBox CboCriterio 
      Height          =   315
      ItemData        =   "FrmConsultaTerceros.frx":0000
      Left            =   120
      List            =   "FrmConsultaTerceros.frx":000A
      Style           =   2  'Dropdown List
      TabIndex        =   3
      Top             =   120
      Width           =   2055
   End
   Begin MSDataGridLib.DataGrid GrillaTerceros 
      Height          =   3615
      Left            =   120
      TabIndex        =   1
      Top             =   480
   ...
   End
End
Attribute VB_Name = "FrmConsultaTerceros"
...
Sub Filtrar()
  ...
  Consulta = "SELECT IdTercero, RazonSocial FROM Terceros"
  If TxtBuscamos <> "" And CboCriterio.Text <> "" Then
    If CboCriterio.Text = "Codigo" Then
      If ChkContenga.Value = 1 Then
        Consulta = Consulta + " where IdTercero like '%" & TxtBuscamos.Text & "%' order by RazonSocial"
      Else
        Consulta = Consulta + " where IdTercero like '" & TxtBuscamos.Text & "%' order by RazonSocial"
      End If
    End If
    If CboCriterio.Text = "Nombre" Then
      If ChkContenga.Value = 1 Then
        Consulta = Consulta + " where RazonSocial like '%" & TxtBuscamos.Text & "%' order by RazonSocial"
      Else
        Consulta = Consulta + " where RazonSocial like '" & TxtBuscamos.Text & "%' order by RazonSocial"
      End If
    End If
  ...
  Set GrillaTerceros.DataSource = rstTem
End Sub
Private Sub GrillaTerceros_KeyPress(KeyAscii As Integer)
  If KeyAscii = 13 Then CmdAceptar_Click
End Sub
Private Sub TxtBuscamos_KeyDown(KeyCode As Integer, Shift As Integer)
  If KeyCode = vbKeyDown Then GrillaTerceros.SetFocus
End Sub
